...
/**
 * An eager genie that will provide your wishes.
 *
 * Wire known dependencies to a callable.
 * All services are resolved by the time the `provide*` methods return.
 *
 * Usage:
 ...
 *   };
 *
 *   $genie = new EagerGenie( $serviceContainer ); // or use Limiter to limit access to certain services only
 *
...
final class EagerGenie
{
    ...
    /**
     * Create an instance of EagerGenie
     * by passing either a Genie or a container implementation instance.
     *
     * @param Genie|self|Container $source
     * @return self
     */
    public static function employ(Genie|self|Container $source): self
    {
        return $source instanceof Genie || $source instanceof self ?
            $source->exposeContainer(function (Container $container): self {
